...
let poll_new_block uri state =
  match Eth.get_block_by_number ~uri ~at_time:(`block (state.block_number + 1)) with
  | None -> None
  | Some block ->
    (match block.Block.number with
     | None ->
       failwith "block has no number!?"
     | Some n ->
       if n <> state.block_number + 1 then
         failwith "block number mismatch"
       else
         Some {
