...
struct
    ...
    let rec loop ctxs =
      ...
      | ctx :: tl ->
        ...
        | _  ->
          ...
          Rpc.Eth.send_contract_and_get_receipt ~uri ~src:account ~data ?gas ?value ()
    in
    ...
    =
      ...
      if siglen <> arglen then
        (let m = Printf.sprintf
         ...
         in
         Lwt_log.debug_f "%s" m;%lwt
         Lwt.fail_with m)
      else begin        
        let method_id = ABI.method_id abi in
        Lwt_log.debug_f
          "calling method %s with code %s\n%!" 
          mname 
        ...
        match gas with
        | Some _ -> 
          Lwt.return { raw_transaction with gas }
        | None ->
  ...
  let call_void_method_tx ~mname ~(src:Types.Address.t) ~(ctx:Types.Address.t) ?gas () =
    let method_id = ABI.keccak_4_bytes mname in
    let data = Bitstr.(Hex.as_string (uncompress method_id)) in
    let tx =
      {
        Types.Tx.src; dst = Some ctx;  gas; gas_price = None; value = None; data; nonce = None
      }
    in
    Lwt.return tx
