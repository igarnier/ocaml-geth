...
sig
  ...
  val get_storage_at : uri:string -> address:Address.t -> position:Z.t -> at_time:time -> string Lwt.t
  val get_transaction_count : uri:string -> address:Address.t -> at_time:time -> int Lwt.t
  val get_transaction_count_by_hash : uri:string -> block_hash:Hash256.t -> int Lwt.t
  ...
  val get_block_by_hash : uri:string -> block_hash:Hash256.t -> Block.t option Lwt.t
  ...
  val get_transaction_receipt : uri:string -> transaction_hash:Hash256.t -> Tx.receipt option Lwt.t
...
sig
  ...
  val send_transaction : uri:string -> src:Address.t -> dst:Address.t -> value:Z.t -> src_pwd:string -> Hash256.t Lwt.t
  val new_account : uri:string -> passphrase:string -> Address.t Lwt.t
  val unlock_account : uri:string -> account:Address.t -> passphrase:string -> unlock_duration:int -> bool Lwt.t
