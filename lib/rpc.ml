...
let print_debug s =
  if !debug_flag then
    Lwt_log.debug_f "Ocaml_geth.Rpc: %s" s
  else
    Lwt.return_unit
...
let rpc_call uri method_name (params : Json.json) =
  ...
  let%lwt resp, body =
    Cohttp_lwt_unix.Client.post (Uri.of_string uri) ~headers ~body
  in
  match body with
  | `Empty ->
    Lwt.fail_with "Ocaml_geth.Rpc.rpc_call: error, empty reply"
  | `Strings ls ->
    Lwt.fail_with "Ocaml_geth.Rpc.rpc_call: error, string list"
  | `String s ->
    Lwt.fail_with "Ocaml_geth.Rpc.rpc_call: error, string"
  | `Stream stream ->
    ...
    (match ls with
     | [] ->
       Lwt.fail_with "Ocaml_geth.Rpc.rpc_call: error, empty reply"
     | [reply] ->
       print_debug (Printf.sprintf "Ocaml_geth.Rpc.call: raw reply =\n%s\n" reply);%lwt
       Lwt.return (Json.from_string reply)
     | _ ->
       Lwt.fail_with "Ocaml_geth.Rpc.rpc_call: error, cannot interpret multi-part reply"
    )
...
let (|>>) promise pure =
  Lwt.bind promise (fun x -> Lwt.return (pure x))
...
struct
  ...
  let estimate_gas ~uri ~transaction =
    try%lwt rpc_call uri "eth_estimateGas" (`List [Tx.to_json transaction]) |>> Get.bigint_as_string
    with
    | exn->
  ...
  let send_transaction_and_get_receipt ~uri ~transaction =
    ...
    let rec loop () =
      ...
      | Some receipt ->
        Lwt.return receipt
      | exception exn ->
        Lwt_log.debug "send_transaction_and_get_receipt: error in get_transaction_receipt\n";%lwt
        Lwt.fail exn
    in
    loop ()
...
struct
  ...
  let stop ~uri =
    rpc_call uri "miner_stop" `Null |>> ignore
