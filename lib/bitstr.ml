...
struct
  ...
  let of_ubigint z =
    if Z.lt z Z.zero then
      failwith "of_ubigint: negative argument"
    else
  ...
  let of_bigint (z : Z.t) =
    ...
    if Z.geq z Z.zero then
      ...
      if Z.fits_int64 z then
        of_int64 (Z.to_int64 z)
      else
        failwith "of_bigint: can't handle two's complement for really big bigints :( TODO"
...
struct
  ...
  let to_signed_bigint s =
    if Bitstring.is_set s 0 then
      ...
      let str = String.map (fun c -> Char.chr ((lnot (Char.code c)) land 0xff)) str in
      let str = String.rev str in
      (* Z.of_bits str *)
      Z.(neg (add (of_bits str) one))
    else
      (* Positive number. Reving the string for endianness purposes. *)
      Z.of_bits (String.rev (Bitstring.string_of_bitstring s))
  ...
  let zero_padding ~(dir:pad_direction) ~bits ~(zeroes : Bits.t) =
    ...
    if zeroes mod 8 <> 0 then
      ...
      match dir with
      | `left ->
        Bitstring.concat [ padding; bits ]
      | `right ->
        Bitstring.concat [ bits; padding ]
  ...
  let take s bits = 
    ...
    (Bitstring.takebits bits s,
     Bitstring.dropbits bits s)
